...
var Data = function () {
    if (typeof SJS.data.cache === 'undefined') SJS.data.cache = {};
}
...
Data.implement('set', function (key, value, type) {
    ...
    if (typeof SJS.data.cache[type] === 'undefined') SJS.data.cache[type] = {};

    if (typeof key.data === 'undefined') key.data = {};

    var cache = SJS.data.cache[type];
    var index = Object.keys(cache).length;

    if (typeof key.data[type] === 'undefined') key.data[type] = [];
...
Data.implement('get', function (key, type) {
    if (typeof key === 'undefined' || typeof key.data === 'undefined') return;
...
Data.implement('remove', function (key, type) {
    if (typeof key === 'undefined' || typeof type === 'undefined') return;

    if (typeof key.data[type] === 'undefined') return;
